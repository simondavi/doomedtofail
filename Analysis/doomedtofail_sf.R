...
# GPA as new variable generated from spSchool-Dataset (Sebastian)
gpa <- spsch %>%
  ...
  rename(abinote = ts11218) %>% # rename
  select(ID_t, gpa)
# other solution
gpa2 <- spsch %>%
  mutate(gpa = ifelse(!is.na(ts11211) & ts11218 <= 4, ts11218, NA)) # GPA if subject-specific higher education entrance qualification (ts11211==1) or general higher education entrance qualification (ts11211==2); otherwise ts11211==NA
# needs to be merged to data4 (not sure if syntax works with data3 since other also other (non-school-) spells are included)
# transformation into categorical variable possible
...
# Operationalization of academic and social integration as in Franz & Paetsch, 2023; see also Dahm et al.; Schaeper, 2022
data5a <- data5 %>% 
  dplyr::mutate(str_aca_int = rowMeans(subset(data5, select = c(tg53211, tg53212, tg53213)),
                                   na.rm = TRUE),
                norm_aca_int = rowMeans(subset(data5, select = c(tg53231, tg53233, tg53235)),
                                   na.rm = TRUE),
                peer_soc_int = rowMeans(subset(data5, select = c(tg53121, tg53122, tg53123)),
                                   na.rm = TRUE),
                fac_soc_int = rowMeans(subset(data5, select = c(tg53111, tg53112, tg53113, tg53114)),
                                   na.rm = TRUE)) 
