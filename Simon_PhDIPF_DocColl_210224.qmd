...
desc_plot <- data6 %>%
  dplyr::filter(!is.na(big_ext))  %>%
  dplyr::filter(!is.na(big_agr))  %>%
  dplyr::filter(!is.na(big_con))  %>%
  dplyr::filter(!is.na(big_neu))  %>%
...
We computed the Latent-Class-Analysis following the guide by [@weller2020]. Model fit was examined based on our theoretical understanding of dropout from teacher education and the BIC (Bayesian Information Criterion) as a statistical criterion, with lower BIC indicating better model fit [@nylund2007]. We will stop evaluating classes, if the BIC increases three subsequent class solutions.
